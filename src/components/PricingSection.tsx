          ...
          {tiers.map((tier) => (
            <article
              ...
              className={`relative bg-[#161616] p-6 border ${
                tier.popular ? "border-[#E6A91A]" : "border-white/10"
              }`}
            >
              {tier.popular && (
                <span className="absolute -top-3 left-1/2 -translate-x-1/2 bg-[#E6A91A] text-[#0A0A0A] text-[10px] tracking-widest uppercase px-3 py-1 font-bold">
                  Most Popular
                </span>
              )}
              <h3 className="font-heading text-xl text-white mb-2">{tier.name}</h3>
              <div className="mb-6">
                <span className="font-heading text-3xl text-[#E6A91A]">
              ...
              <ul className="space-y-3" role="list">
                {tier.features.map((feature) => (
                  <li key={feature} className="flex items-start gap-3">
                    <Check
                      className="w-4 h-4 text-[#E6A91A] flex-shrink-0 mt-0.5"
                      aria-hidden="true"
                    />
